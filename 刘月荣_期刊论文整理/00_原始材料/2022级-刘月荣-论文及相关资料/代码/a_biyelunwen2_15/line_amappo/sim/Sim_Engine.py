...
class Engine():
    def __init__(self, bus_list,busstop_list,route_list,simulation_step,dispatch_times, use_state_norm,use_reward_norm,use_reward_scaling,demand=0,agents=None,share_scale=0, is_allow_overtake=0,hold_once_arr=1,control_type=1,seed=1,all=0,weight=0,ppo_epoch=3,num_mini_batch=256,device=None):
        # self.device = device
    ...
    def cal_statistic(self,name,train=1):
        ...
        origins = []
        dests = []
        still_wait = 0     # 未上车的乘客数量？
        stop_wise_wait = {}    # 记录每个站点所有乘客的等待时间
        stop_wise_hold = {}    # 记录每个站点所有车辆的驻站时间
        delay = []           # 乘客延误时间：乘客的下车时间-乘客的到达时间，未乘客总的行程时间，总的行程时间-乘客在路上时间，即为乘客等待车辆到来和驻站控制产生的延误
        ...
        stop_wise_wait_order = []  # 记录的是每个站点乘客的平均等待时间
        stop_wise_hold_order = []  # 记录的是每个站点车辆的平均驻站时间

        arr_times = []   # 每个站点：所有的有车辆到达的时间步
        buslog = pd.DataFrame()
        for bus_stop_id in bus.pass_stop:
            buslog[bus_stop_id]=self.busstop_list[bus_stop_id].arr_log[bus.route_id]    # arr_log记录的是公交车到站的时间   每个公交站点所有有车辆到达的时间步
            ...
            for k,v in self.busstop_list[bus_stop_id].arr_log.items():
                h =  np.array(v )[1:]  -  np.array(v)[:-1]   # 后一辆车的到站时间与前一辆差值（每两辆车之间都计算）
        ...
        for k in bus.pass_stop:
            data = [np.array([arr]) if not isinstance(arr, np.ndarray) else arr for arr in stop_wise_hold[k]]   # 每个元素arr，代码首先检查arr是否是numpy数组（np.ndarray）。如果arr不是numpy数组，那么它会被封装在一个数组中，即np.array([arr])。如果arr已经是numpy数组，那么它会保持不变
    ...
    def serve(self, bus, stop):
        ...
        if bus != None:
            ...
            for t in new_arr:  # self.pax_list列表中
                ...
                self.pax_list[num].dest = random.choice(bus.left_stop[1:])  # 设置乘客的下车站点___________________________________________________设置了乘客的下车站点
            ...
            for num in waitinglist:  # waitinglist是在该站点等待上车的乘客。self.pax_list是包含整个环境中所有的乘客
                ...
                if bus != None and bus.capacity - len(bus.onboard_list) > 0 and self.pax_list[
                    num].route == bus.route_id:  # 还未超过车辆的容纳能力
                    self.pax_list[
                        num].onboard_time = self.simulation_step  # 设置乘客的上车时间——当车辆容量够时，在站点等待的乘客的上车时间是相同的,到站实践是不同的——————————————————————————————————————————设置乘客上车时间
    ...
    def sim(self):
            ...
            if bus.is_dispatch == 0 and bus.dispatch_time <= self.simulation_step:  # self.simulation_step:24149  is_dispatch=0表示公交车还未发出
                ...
                if bus.is_virtual != 1:
                    if bus.speed_type == 1:
                        ...
                        bus.current_speed = bus.speed * np.random.randint(80., 120.) / 100.  # 随机数不包括120
                        # bus.current_speed = np.random.normal(28/3600, 1/3600, 1)
                else:
                    bus.current_speed = bus.speed * 0.8
                self.dispatch_buslist[bus_id] = bus

            # 这个if条件是用来判断公交车剩余未经过的站点是否小于等于0，就是是否运行完了一个行程
            if bus.is_dispatch == 1 and len(
                    self.dispatch_buslist[bus_id].left_stop) <= 0:  # 初始时，self.dispatch_buslist列表是空的，依赖于上面的if来填充列表
        ...
        for bus_id, bus in self.dispatch_buslist.items():    # 遍历每一个已经发车的车辆
            ...
            if bus.is_virtual==1 and bus.arr==0 and abs(bus.loc[-1]-bus.stop_dist[bus.left_stop[0]])<bus.current_speed :
            ...
            if bus.is_virtual==0 and bus.arr==0 and abs(bus.loc[-1]-bus.stop_dist[bus.left_stop[0]])<bus.current_speed :   # 如果公交车的实时位置与将要到达的站点之间的距离小于公交的速度，就让公交车到站, bus_stop_dist记录的是每个公交站的位置
                ...
                bus.serve_remain = max(board_cost,alight_cost)    # 表示公交车在站点的服务时间（但为什么要加1）
            ...
            # 表示公交车要离开站点
                ...
                if self.is_allow_overtake == 1:    # =1应该是允许超车
                    ...
                    if bus.forward_bus in self.dispatch_buslist and bus.current_speed+bus.loc[-1]>=self.dispatch_buslist[bus.forward_bus].loc[-1]:
                        bus.stop()  # 也就是将要发生超车时，让公交车停车，禁止超车
                        bus.current_speed = bus.speed * np.random.randint(80, 120) / 100.
                        # bus.current_speed = np.random.normal(25/3600, 1/3600, 1)
    ...
    def control(self,bus,bus_stop,type=0):
        ...
        if type == 1:  # fh
            ...
            if bus.forward_bus == None:
                ...
                return max(0, min(self.max_hold_time,5.8*60 + 0.8 * (abs(bus.dispatch_time-self.bus_list[bus.forward_bus].dispatch_time) - fh_)))#2020年论文中的取值
        # 强化学习进行驻站控制
        ...
        if type == 4:  # HH   固定车头时距的控制——当前向车头时距小于期望的车头时距时进行驻站控制d = H0-h-
            ...
            if bus.forward_bus == None:
                ...
                return max(0, min(self.max_hold_time, bus.dispatch_time-self.bus_list[bus.forward_bus].dispatch_time - fh_))
    ...
    def rl_control(self, bus, bus_stop):
        ...
        if len(self.GM.temp_memory[bus.id]['a']) > 0:
            ...
            # bus_dist = [0.]
            fp = [state + [0.] + [0.] + [0.] + [0.] + [bus.id]]
            temp = bus.arr_time_list[-2]    # 最初的bus.last_vist_interval 为公交的发车时间
            while temp <= current_interval:  # 为了寻找这个在这个公交上一次驻站控制到这一次进行驻站控制期间，其他进行驻站控制的公交
                if temp in self.arrivals:  # self.arrivals记录相应时间步，进行驻站控制的车辆、公交站、公交的在车人数
                    for record in self.arrivals[temp]:
                        bus_stop_id_ = record[0]
        ...
        if self.share_scale == 1:    # 默认设置值为1，应该表示参数是共享的
            action, action_log_prob = np.array(self.agents[bus.route_id].choose_action([state,fp]))
        # mark记录了状态动作对
        '''将这儿进行更改：mark和his不记录动作'''
        ...
        return int(180. * action)  # 最终返回的是驻站时间
    ...
    def learn(self):
        ...
        if self.share_scale == 1:  # true
            '''为了计算每个智能体的优势函数，累积回报'''
            obs, w_obs, feature_p, actions, old_action_log_probs, value_pred, returns, adv_targ = [], [], [], [], [], [], [], []
            for rid, r in self.route_list.items():  # 只有一次循环
                # for b in r.bus_list:   # 不需要遍历字典的键，只需要遍历值就可以，这样可以节省时间
                for _ in self.GM.temp_memory.values():
                    if len(_['s']) <= 2:
                        continue
                    ...
                    # 计算状态价值
                    values_ = self.agents[rid].critic(
                        torch.tensor(ws_, dtype=torch.float32).reshape(len(ws_), -1)).cpu().detach()
                    values = values_.numpy()  # 包含了当前状态价值 和 下一状态价值
                    value_pred += values_.tolist()[:-1]
                    r_ = _['r'][:-1]
                    fp_ = [torch.tensor(i, dtype=torch.float32) for i in _['fp']]
            ...
            def feed_forward_generator(obs, w_obs, feature_p, actions, old_action_log_probs, value_pred,
                ...
                for index in BatchSampler(SubsetRandomSampler(range(batch_size)), num_mini_batch, True):
                    ...
                    feature_p_batch = feature_p[index].tolist()
                    ...
                    # next_feature_p_batch = next_feature_p[index].tolist()
                    value_pred_batch = value_pred[index]
                    returns_batch = returns[index]
                    adv_targ_batch = adv_targ[index]
                    yield obs_batch, w_obs_batch, feature_p_batch, actions_batch, old_action_log_probs_batch,  value_pred_batch, returns_batch, adv_targ_batch
            ...
            # batch_size = len(obs)
                ...
                #     # next_feature_p_batch = next_feature_p[indices].tolist()
            ...
            for rid, r in self.route_list.items():
                for _i in range(self.ppo_epoch):
                    data_generator = feed_forward_generator(obs, w_obs, feature_p, actions, old_action_log_probs,
                ...
                num_updates = self.ppo_epoch * (obs.shape[0] // self.num_mini_batch)
        ...
        #     for rid,r in self.route_list.items():  # 只有一次循环
        #         for id_ in r.bus_list:
        #             while len(self.GM.memory[id_]) <= 0:
        #                 break
        #             reward_,value_,next_value_,returns_,adv_targ_ = [],[],[],[],[]
        #
        #             for s, fp, a, a_log_prob, r, ns, nfp,value,next_value in self.GM.memory[id_]:
        #                 obs.append(s)
        #                 feature_p.append(torch.tensor(fp,dtype=torch.float32))
        #                 actions.append(a)
        #                 old_action_log_probs.append(a_log_prob)
        #                 next_feature_p.append(torch.tensor(nfp,dtype=torch.float32))
        #                 value_pred.append(value)
        #
        #                 reward_.append(r)
        #                 value_.append(value)
        #                 next_value_.append(next_value)
        #
        #             gae = 0
        #             for i_ in reversed(range(len(reward_))):   # 计算每个智能体每一步的优势函数和累积回报
        #                 delta = reward_[i_] + self.agents[rid].gamma * next_value_[i_] - value_[i_]
        #                 gae = delta + self.agents[rid].gamma * self.agents[rid].gae_lambda * gae
        #                 return_ = gae + value_[i_]
        #                 self.GM.add(return_,gae,id_)   # 将计算出来的回报和优势值存储在经验回放池中
        #                 returns_.insert(0,return_)
        #                 adv_targ_.insert(0,gae)
        #             adv_targ_copy = adv_targ_.copy()
        #             mean_advantages = np.mean(adv_targ_copy)  # 忽略nan值  计算（非缺失值）元素的平均值
        ...
        #     adv_targ = torch.tensor(adv_targ,dtype=torch.float)
        #
        #     def feed_forward_generator(obs,feature_p,actions,old_action_log_probs,next_feature_p,value_pred,returns,adv_targ,num_mini_batch):
        #         batch_size = len(obs)
        #         mini_batch_size = batch_size // num_mini_batch  # 16是batch
        #         rand = torch.randperm(batch_size).numpy()
        #         sampler = [rand[_ * mini_batch_size:(_ + 1) * mini_batch_size] for _ in range(num_mini_batch)]
        ...
        #             adv_targ_batch = adv_targ[indices]
        #
        #             yield obs_batch, feature_p_batch, actions_batch, old_action_log_probs_batch, next_feature_p_batch, value_pred_batch, returns_batch, adv_targ_batch
        #
        #     train_info = {}
        #     train_info['value_loss'] = 0
        #     train_info['policy_loss'] = 0
        ...
        #     for rid, r in self.route_list.items():
        #         for _i in range(self.ppo_epoch):
        #             data_generator = feed_forward_generator(obs,feature_p,actions,old_action_log_probs,next_feature_p,value_pred,returns,adv_targ,self.num_mini_batch)
        #             for sample in data_generator:
        #                 value_loss, policy_loss, imp_weights = self.agents[rid].learn(sample)
        #                 train_info['value_loss'] += value_loss.item()
        #                 train_info['policy_loss'] += policy_loss.item()
        #                 # train_info['dist_entropy'] += dist_entropy.item()
        #                 train_info['ratio'] += imp_weights.mean()
