...
class Actor(nn.Module):
    def __init__(self, input_size, hidden_size=400, output_size=1,seed=1):
        ...
        self.u_attentions = [
            layers.GraphAttentionLayer(input_size +1, self.hidden, dropout=0.1, alpha=0.2, concat=True)
            for _ in range(1)]
        for i, attention in enumerate(self.u_attentions):
            self.add_module('attention_{}'.format(i), attention)
        self.u_out_att = layers.GraphAttentionLayer(self.hidden * 1, self.hidden, dropout=0.1, alpha=0.2,
                                                    concat=False)
        self.d_attentions = [
            layers.GraphAttentionLayer(input_size+1 , self.hidden, dropout=0.1, alpha=0.2, concat=True)
            for _ in
        ...
        for i, attention in enumerate(self.d_attentions):
            self.add_module('attention_{}'.format(i), attention)
        self.d_out_att = layers.GraphAttentionLayer(self.hidden * 1, self.hidden, dropout=0.1, alpha=0.2,
                                                    concat=False)
        self.elu = nn.ELU()
        self.tanh = nn.Tanh()
