...
def prepare_eg(fp):
    ...
    for i in range(len(fp)):  # 分为上下游车辆，上游车辆，晚于当前车辆发车
        fp_ = fp[i][(fp[i][:,
                     -3] <= 0)]  # 将满足(fp[i][:, -3] <= 0)条件的张量选择出来赋值给fp_,[:,-3]是选择张量中倒数第三列的所有行    这儿的意思是寻找倒数第三列（两车之间的距离=当前车辆-相邻车辆） 等于0表示自身节点特征，小于0时，表示相邻车辆比当前车辆后发车，属于上游事件
...
class Actor(nn.Module):
    ...
    def forward(self, s):
        ...
        mean = abs(self.tanh(self.linear3(x)))
        std = self.softplus(self.linear4(x))
        return mean,std
