...
import math
matplotlib.rc("font",**{"family":"sans-serif","sans-serif":["Helvetica",'SimSun',"Arial"],"size":14})
...
parser.add_argument("--model", type=str, default='mappo')  # caac  ddpg maddpg
...
parser.add_argument("--episode", type=int, default=21)  # training episode
...
parser.add_argument("--control", type=int, default=3)  # 0 for no control;  1 for FH; 2 for RL (MAPPO, AMAPPO); 3 for BH ;4 for HH ;5 SH
...
def train(args):
    ...
    for ep in range(args.episode):    # args.episode=401
        ...
        name = abspath + "/log/" + args.data + args.model + str('_') + str(args.control) + str('_')+ str(args.restore) + str('_')
...
def evaluate(args):
        ...
        if args.share_scale == 1:  # true
            ...
            for k, v in eng.route_list.items():  # 只有一条线路  线路为一个智能体
                agent = Agent(state_dim=state_dim, name='', n_stops=len(bus_stop_list), buslist=bus_list,
                              seed=args.seed)  # 每个agent都有一个crtic一个target_crtic,以及一个actor一个target_actor
                agents[k] = agent
                agents[k].actor.load_state_dict(torch.load(r"D:\Edge down\pycharm\Py_Projects\a_biyelunwen\base_mappo\model\save\_A_0_1_1_2_mappo_caac_1_actor.pth"))
                agents[k].critic.load_state_dict(torch.load(r"D:\Edge down\pycharm\Py_Projects\a_biyelunwen\base_mappo\model\save\_A_0_1_1_2_mappo_caac_1_critic.pth"))
    # print(agents['0_1'].actor.state_dict())
    # print(agents['0_1'].critic.state_dict())
    for ep in range(20):    # args.episode=401
        ...
        log, _, _, _, _ = eng.cal_statistic(
            ...
            train=args.train)
        abspath = os.path.abspath(os.path.dirname(__file__))
        name = abspath + "/logt/" + args.data + args.model + str('_') + str(args.control) + str('_')
