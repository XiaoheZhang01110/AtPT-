...
class Engine():
    ...
    def control(self,bus,bus_stop,type=0):
        ...
        if type == 1:
            ...
            if bus.forward_bus == None:
                ...
                return max(0, min(self.max_hold_time, 58 + 0.05 * (abs(bus.dispatch_time-self.bus_list[bus.forward_bus].dispatch_time) - fh_)))
        # 强化学习进行驻站控制
        ...
        if type == 3:  # BH
            if bus.backward_bus == None:
                ...
                return max(0, min(self.max_hold_time, 58 + self.alpha * (self.bus_list[bus.backward_bus].dispatch_time - bus.dispatch_time - 360.)))  # 这儿要不要减360.
        if type == 4:  # HH   固定车头时距的控制——当前向车头时距小于期望的车头时距时进行驻站控制d = H0-h-
            # fh = bus_stop.bus_arr_interval[-1]
            if bus.forward_bus == None:
                return 0
            else:
                return max(0, min(self.max_hold_time, 360. - fh))
                ...
                if temp in self.arrivals:  # self.arrivals记录相应时间步，进行驻站控制的车辆、公交站、公交的在车人数
                    for record in self.arrivals[temp]:
                        ...
                        if (bus_id_ == bus.forward_bus or bus_id_ == bus.backward_bus) or (
                            ...
                            bus_dist = [(curr_bus - neigh_bus) / len(self.bus_list)]  # 发车时间索引之差/公交数量(两控制点之间公交车的数量)
                            stop_dist = [
                                (bus.stop_list.index(bus.pass_stop[-1]) - bus.stop_list.index(bus_stop_id_)) / len(
                                    self.busstop_list)]  # 当前公交上一次到站的站点索引与相邻公交主站控制的索引之差/公交站数量(两控制点之间公交站的数量)
                            fp.append(self.bus_list[bus_id_].his[temp] + stop_dist + bus_dist + [
                                abs(temp - current_interval)] + [
                                          bus_id_])  # 边际贡献：相邻公交站驻站是的状态动作+bus_dist+stop_dist+当前时间步与该公交站驻站时间步之差+相邻公交的索引
    ...
    def cal_headway(self, bus):
        ...
        if bus.backward_bus != None and bus.backward_bus in self.dispatch_buslist:
            ...
            if self.dispatch_times[bus.route_id].index(bus.dispatch_time) == len(
                    self.dispatch_times[bus.route_id]) - 1:  # 表示该车是该线路上最后一个发出的车辆
                bh = 0.
        ...
        for bus_id, bus_ in self.dispatch_buslist.items():  # 遍历 所有 已经发出的公交车列表
            if bus_.route_id == bus.route_id:
                if bus_.forward_bus != None and bus_.forward_bus in self.dispatch_buslist:  # 公交前面有车，且公交前面的车的位于已经发出车辆的列表中
                    fh.append(
                        abs(bus_.loc[-1] - self.bus_list[bus_.forward_bus].loc[-1]) / bus_.speed)  # 将公交的实际车头时距放入fh列表中
