...
class Agent():
    ...
    def learn(self, memories, batch=16):
        ...
        for s, fp, a, r, ns, nfp, in memory:   # 主要的操作就是将状态放在一起，动作放在一起，。。。。
            ...
            _fp_[0, self.state_dim+1] = self.actor(torch.tensor(s, dtype=torch.float32)).detach()   #将_fp_中的第0行第4列的数据替换   # tensor([0.0836])
            batch_fp_critic_t.append(_fp_)
            batch_actor_a.append(self.actor(torch.tensor(s, dtype=torch.float32)))   # 包含有梯度的   # tensor([0.0836], grad_fn=<EluBackward0>)
