...
class Bus_stop():
    def __init__(self,id,lat,lon  ):
        ...
        self.arr_everybus_load = {}  # 记录到达公交站的每辆公家车上的人数
        for _ in range(6):
            self.arr_everybus_load[_] = []
    ...
    # 生成乘客采用此方法
    def pax_gen_od(self,bus,sim_step=0):

        base=0
        interval = 0

        if bus!=None:   # true
            if len(self.arr_log[bus.route_id])>1:   # 表示该站点并不是第一次有车辆到站，self.arr_log记录的是到站时间,当前车辆的到达时间已经记录
                # 连续两辆车到达该公交站的时间间隔
                interval = (self.arr_log[bus.route_id][-1] -self.arr_log[bus.route_id][-2] )    # 同一路线上两辆车到达同一站点的时间间隔，也就是说乘客的聚集时间是该辆车到站时间与上一辆车到站时间的差值
                sample = (np.random.poisson(self.rate,int(interval)))   # np.random.poisson()函数的完整形式是np.random.poisson(lam, size=None)，其中lam是泊松分布的参数λ(单位时间内事件发生的平均次数)，size是生成随机数样本的大小
                base=self.arr_log[bus.route_id][-2]      # 公交站上一次公交车到站的时间步
                pax = []    # 记录的是乘客到达公交站的时间，长度为乘客数量
                for i in range(sample.shape[0]):
                    if sample[i]>0:
                        pax+=[base+i for t in range(sample[i])]
            else:   # 表示该站点第一次有车辆到站，乘客最早在车辆到达前4分钟聚集       # np.random.poisson(lam=2, size=(3, 4))  lam，表示泊松分布的均值或参数λ;size，表示输出随机数的形状
                # assume passenger will gather in less than 15min before the first bus began.    # sim_step代表仿真的时间步长
                sample = (np.random.poisson(self.rate,240))    # 生成长度为900的一维数据,900s=15min,表示乘客最早在车辆到达前15min中开始聚集

                base = self.arr_log[bus.route_id][-1]  # 该线路上最后一次到站的时间,也就是当前公交车的到站时间
                pax = []    # 列表中的元素是用来表示乘客生成的时间，列表长度表示乘客数量
                for i in range(sample.shape[0]):
                    if sample[i]>0:
                        pax+=[base-i for t in range(sample[i])]   # pax列表的长度为乘客的数量，pax的元素为每个乘客的到达站点的时间
        else:
