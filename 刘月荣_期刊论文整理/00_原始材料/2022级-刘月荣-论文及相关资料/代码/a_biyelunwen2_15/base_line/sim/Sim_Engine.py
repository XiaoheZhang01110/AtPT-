...
class RewardScaling:
    ...
    def reset(self):
        self.R = np.zeros(self.shape)
class Engine():
    def __init__(self, bus_list,busstop_list,route_list,simulation_step,dispatch_times, demand=0,agents=None,share_scale=0, is_allow_overtake=0,hold_once_arr=1,control_type=1,seed=1,all=0,weight=0,ppo_epoch=5,num_mini_batch=4):
    ...
    def sim(self):
        ...
        for bus_id, bus in self.dispatch_buslist.items():    #
            ...
            if bus.is_virtual==0 and bus.arr==0 and abs(bus.loc[-1]-bus.stop_dist[bus.left_stop[0]])<bus.speed :   # 如果公交车的实时位置与将要到达的站点之间的距离小于公交的速度，就让公交车到站, bus_stop_dist记录的是每个公交站的位置
                ...
                alight_cost,board_cost = self.serve(bus,curr_stop)   # 公交到站时，上下客共需要花费多长时间
                # 更新公交车辆的状态
    ...
    def control(self,bus,bus_stop,type=0):
        ...
        #     if bus.arrival_schedule - self.simulation_step > 0:
        # #         return min(self.max_hold_time, bus.arrival_schedule - self.simulation_step)
        #     else:
        #         return 0
        return 0
        ...
        #         return max(0, min(self.max_hold_time,5.8*60 + 0.8 * (480. - fh)))
        # # 强化学习进行驻站控制
        # if type==2:   # 类型2应该指的是使用强化学习进行驻站控制
        #     return self.rl_control(bus,bus_stop)     # 返回的是驻站时间
        # if type==3:   # BH
        ...
        #         return 0
        #     else:
        #         return max(0,min(self.max_hold_time,self.alpha * (bh-360.))   # 这儿要不要减360.
        # if type == 4:   # HH
        #     fh_ = bus_stop.bus_arr_interval[-1]
        #     if bus.forward_bus == None:
        #         return 0
    ...
    def rl_control(self, bus, bus_stop):
        ...
        if self.share_scale == 1:    # 默认设置值为1，应该表示参数是共享的
            action,action_log_prob = np.array(self.agents[bus.route_id].choose_action(np.array(state).reshape(-1, )))
        ...
        if len(self.GM.temp_memory[bus.id]['a']) > 0:    # temp_memory是一个字典，里面存储了s,a,fp,r
            ...
            fp = [self.GM.temp_memory[bus.id]['s'][-1] + stop_dist + bus_dist + [0.] + [bus.id]]  # stop_dist + bus_dist + [0.]这儿的0是为了确保相同的特征长度
            ...
            # reward1 = -reward_scaling(reward1)
            # reward2 = -reward_scaling(reward2)
    ...
    def route_info(self,bus):
        fh = [480 for _ in range(3)]  # fh = [500,500,500]
        bh = [480 for _ in range(3)]  # bh = [500,500,500]
    ...
    def learn(self):
        ...
        if self.share_scale == 1:  # true
            '''为了计算每个智能体的优势函数，累积回报'''
            obs, feature_p, actions, old_action_log_probs, next_feature_p, value_pred, returns, adv_targ = [], [], [], [], [], [], [], []
            for rid, r in self.route_list.items():  # 只有一次循环
                # m = 0
                # self.bus_hash = {}  # 为每个公交车排序r
                # for k in r.bus_list:
                #     self.bus_hash[k] = m
                #     m += 1
                ...
                for _ in self.GM.temp_memory.values():   # 分别为每辆车的数据
                    ...
                    whole_s = []  # 最终会有 与 s_长度相同的列表

                    m = 0
                    self.bus_hash = {}  # 为每个公交车排序r
                    for k in r.bus_list:
                        if k != _['fp'][0][0][-1]:  # 当前车辆的id
                            self.bus_hash[k] = m
                            m += 1

                    for i_ in (_['fp']):

                        ws = [[0., 0., 0.] for i in range(len(r.bus_list) - 1)]

                        if len(i_) <= 1:  # 只有自身节点特征
                            ...
                            for n in range(len(i_)):
                                fp_ = i_[n]  # 节点特征
                                b__ = i_[n][-1]  # 车辆id
                                gap = i_[n][-2]  # 两车控制时间之差（自我智能体于与其他智能体）

                                if n > 0 and gap == 0:  # i_>0表示非自我智能体，gap==0表示辆车辆同时发生驻站控制

                                    ws[self.bus_hash[b__]] = fp_[:3]  #
                            whole_s.append(ws)

                    # fp_ = [torch.tensor(i, dtype=torch.float32) for i in _['fp']]  # 与状态相同(列表中存储的每个元素都是张量)

                    # 计算状态价值
                    values_ = self.agents[rid].critic([torch.tensor(s_, dtype=torch.float32),
                                                       torch.tensor(whole_s, dtype=torch.float32).reshape(len(s_),
                                                                                                          -1)]).cpu().detach()
                    # values_ = self.agents[rid].critic(torch.tensor(whole_s, dtype=torch.float32).reshape(len(s_),-1)).cpu().detach()
            ...
            def feed_forward_generator(obs, feature_p, actions, old_action_log_probs, next_feature_p, value_pred,
            ...
            for rid, r in self.route_list.items():
                for _i in range(self.ppo_epoch):
                    data_generator = feed_forward_generator(obs, feature_p, actions, old_action_log_probs,
                                                            next_feature_p, value_pred, returns, adv_targ,
                                                            self.num_mini_batch)
