    ...
    def serve(self,bus,stop):
        ...
        if bus!=None:
            ...
            for t in sorted(new_arr):  # self.pax_list列表中
                ...
                # 为乘客设置下车站点
                s = np.random.randint(1, len(self.busstop_list))  # 包含1，不包含len(self.busstop_list)
                alight_index = (int(stop.id) + s) % len(self.busstop_list)  # 取余
    ...
    def rl_control(self, bus, bus_stop):
        ...
        if len(self.GM.temp_memory[bus.id]['a']) > 0:
            ...
            while temp <= current_interval:  # 为了寻找这个在这个公交上一次驻站控制到这一次进行驻站控制期间，其他进行驻站控制的公交
                ...
                temp += 1
            reward1 = (-var / mean / mean) * (1 - self.weight) * 1   # 5是什么？    # 论文公式二的第一项（多乘了5）
