...
class Critic(nn.Module):
    def __init__(self, state_dim, nheads=1, n_stops=12,n_buses=6):
        ...
        # for ego critic
        self.fc0 = nn.Linear(state_dim , self.hidden)   # 自身状态
        self.fc1 = nn.Linear(state_dim * (n_buses-1), self.hidden)
...
class Agent():
    def __init__(self, state_dim, name, seed=123, n_stops=22, buslist=None):
        ...
        self.critic_optim = torch.optim.Adam(self.critic.parameters(), lr=0.001,eps=1e-5) # eps=1e-4   # 可以使用 self.critic_optim 对象来对模型参数进行优化，例如调用 self.critic_optim.step() 来更新参数
    ...
    def learn(self, sample):
        ...
        def actor_learn():   # 这里没有计算交叉熵
            action_log_probs = []
            dist_entropy = []
            action_logit = self.actor(obs_batch)
            ...
            return policy_loss,imp_weights,values,dist_entropy
        ...
        policy_loss, imp_weights,values,dist_entropy = actor_learn()
