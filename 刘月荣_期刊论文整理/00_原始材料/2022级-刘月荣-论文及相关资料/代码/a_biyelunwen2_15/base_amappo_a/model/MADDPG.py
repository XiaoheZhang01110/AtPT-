...
class Agent():
    ...
    def learn(self, memories, batch=16 ,bus_id=None):
        ...
        for s, fp, a, r, ns, nfp in memory:
            ...
            batch_r.append(r)
            batch_ns.append(ns)
            wa = [0. for _ in range(len(self.buslist)-1)]  # 用来存储除自身外其他智能体的动作
            for i_ in range(len(fp)):
                fp_=fp[i_]   # 节点特征
                b = fp_[-1]  # 车辆id
                gap = fp_[-2]  # 两车控制时间之差（自我智能体于与其他智能体）

                if i_>0 and gap==0:   # i_>0表示非自我智能体，gap==0表示辆车辆同时发生驻站控制
                    wa[self.bus_hash[b]]=fp_[self.state_dim ]   # 替换为动作 这儿是为了考虑其他所有智能体的动作，若其他智能体未发生驻站，填充为0
            whole_a.append(wa)
        ...
        # update critic
        Q = self.critic([b_s, wb_a, b_a])   # 考虑了所有其他智能体的动作 （没考虑异步特性）
        q_target = b_r + self.gamma * self.critic_target([b_s_, wb_na, self.actor_target(b_s_).detach() ]).detach()   # 目标Q值中使用了目标Q网络，目标Q网络中需要的动作值是目标动作网络计算出来的
        loss_fn = nn.MSELoss()
        ...
        # update actor
        policy_loss = self.critic([b_s, wb_a, self.actor(b_s)])    # Q值中的动作是动作网络计算的   所求的动作是Q值最大的动作

        # take gradient step
        self.actor_optim.zero_grad()
        policy_loss = -policy_loss       # 损失函数是Q的负值
        ...
        def soft_update(net_target, net, tau=0.02):
            for target_param, param in zip(net_target.parameters(), net.parameters()):
                target_param.data.copy_(target_param.data * (1.0 - tau) + param.data * tau)   # w_ <—— tau * w + (1-tau) * w_
