...
class Actor(nn.Module):
    ...
    def event_critic(self, fp):
        ...
        for i in range(len(u_adjs)):
            ...
            if d_adj.size(0) >= 2:
                d_x = self.d_egat(d_x, d_adj)    # 下游所有特征的求和
            else:
            ...
            # a.append(self.tanh(self.linear2(u_x + d_x)))  不考虑
            # a.append(self.linear2(u_x + d_x))  #相对来说比较好
            a.append(self.linear2(torch.cat([u_x , d_x],1)))  # 优秀
            # a.append(self.tanh(self.linear2(torch.cat([u_x, d_x], 1)))) # 不好

        # q = torch.stack(a, 0).view(-1, 1)
        q = torch.stack(a, 0).squeeze(1)
    ...
    def forward(self, xs):
        s,fp = xs
        q = self.event_critic(fp)
        # s = self.tanh(self.linear1(s))
        s = self.linear1(s)
        x = torch.cat([s, q], 1)
        x = self.tanh(self.linear3(x))      # 合并后再使用激活函数更好
        mean = abs(self.tanh(self.linear4(x)))
        std = self.softplus(self.linear5(x))
        return mean,std
