...
parser.add_argument("--episode", type=int, default=201)  # training episode
...
def train(args):
    stop_list, pax_num = U.getStopList(args.data)    # stop_list=46,字典   pax_num=0
    print('Stops prepared, total bus stops: %g' % (len(stop_list)))
    bus_routes = U.getBusRoute(args.data)    # 0_1:里面包含59辆公交车
    ...
    for ep in range(args.episode):    # args.episode=401
        ...
        for _, stop in stop_list_.items():
            stop.set_rate(r)       # 为每个公交站都设置了一个相同的到达率（乘客）
