...
class GraphAttentionLayer(nn.Module):
    ...
    """
    # 构造函数，初始化图注意力层参数
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):  # dropout的概率，用于控制特征的随机失活
    ...
    def forward(self, h, adj):   # h是节点特征矩阵，adj是图的邻接矩阵，表示节点之间的连接关系
        ...
        e = F.elu(torch.matmul(a_input, self.a).squeeze(2))    # 计算注意力系数，并使用激活函数进行非线性处理  应该就算是σ(f()) squeeze(2)去除第二维度    self.a的size(800,1)
        ...
        return F.elu(h_prime)   # 对加权后的特征表示h_prime应用指数线性激活函数，并作为输出返回
