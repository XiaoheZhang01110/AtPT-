...
class bus_stop():
    ...
    def __init__(self,alpha,radius,id,arr_rate=1/2/30, board_rate=0.33, arr_dis=0, H=60*6,wait_num=0): # arr_dis : arrive distribution. 0 for uniform, 1 for possion
        self.id = id
        self.arr_dis = arr_dis  # 到达率分布 0——泊松 1——均匀
        self.schedule_hw = H # scheduled bus arrival headway   # 计划的车头时距的间隔
        self.actual_bus_arr=[]   # 公交车实际到达的时间列表
        self.bus_arr_interval = []   # 公交车实际到达的间隔列表
        self.arr_rate = arr_rate # min(abs(np.random.normal(loc=1/2/30,scale=0.1)),2/60 )# person / s
        # print(self.arr_rate)
        self.tor_cap = 120 # bus capacity
        self.board_rate = board_rate  # person / s   # 每秒钟多少个乘客
        self.wait_num = wait_num # 1 pixel for 10 person
        self.corr_radius = radius   # 公交走廊的半径
        self.loc = alpha # determine stop location  公交站的确定位置
    ...
    def arrive(self):
        ...
        if self.arr_dis==0:
            ...
            # self.wait_num_sep+=k
            # self.wait_num_all+=k
            k = np.random.poisson(self.arr_rate, 1)[0]   # 生成一个期望值和方差都为self.arr_rate的随机数
