...
class Env():
    ...
    def __init__(self, state_dim, action_dim, bus_num, bus_stop_num, r, emit_time_list,
                 bus_dep_list, bus_stop_loc_list, update_step= 1, sim_horizon=60*60*3,control_id=[1,3,5],train_mode=0):
        ...
        if train_mode==1:
            self.init_stopindex = [np.random.multinomial(1, [1. / 12 for i in range(12)], 1).tolist()[0].index(1) for j in range(6)]
        ...
        if train_mode == 0:   # 公交位置是等间距的
            dispatch_loc = np.pi * 2 / 12 * (self.bus_num - 0 - 1) * 2
            for i in range(bus_num):   # 实例化6个公交
                b = bus(w=np.pi/(3.6*400) , capacity=72, radius=r, id=i, stop_nums=bus_stop_num,
                        color_='blue', emit_time=emit_time_list[i],dispatch_loc=dispatch_loc)
                # dispatch_loc = [300°,240°,180°,120°,60°,0°]
                dispatch_loc -= np.pi * 2/12 * 2
                self.bus_list.append(b)
        arr_rates = [1 / 60 / 2, 1 / 60 / 2, 1 / 60 / 1.2, 1 / 60, 1 / 60, 1 / 60 * 3, 1 / 60 * 4, 1 / 60 * 2, 1 / 60,
                     1 / 60 / 1.5, 1 / 60 / 1.8, 1 / 60 / 2]  # 每个公交站的乘客到达率  passenger/s
        for i in range(bus_stop_num):   # 实例化12个公交站
    ...
    def sim(self, ):
        ...
        for b in self.bus_list:
            ...
            # 每个sim步更新公交的运行状态，决定公交车是在运动，停车服务还是在驻站控制
            b.is_serving=0
        ...
        # if self.bus_list[-1].is_emit == True:
             # RL control whenever a bus pass the control point
             if sum(p_flag)>0:
                is_control=0
                    ...
                    for bb in self.bus_list:
                        for c in self.control_id_set:
                            if self.bus_loc_stop[bb.id] ==-1 and ((bb.dispatch_loc > self.bus_stop_loc_list[c] and abs(
                                ...
                                self.bus_loc_stop[bb.id] = c      # 这儿就是当出现is_control=1是，记录公交达到的站点
    ...
    def control(self, action):
        ...
        for b in self.bus_list:
            b.is_serving_rl = 0
            if b.special_state==1:     # 当发生串车，也就是后车追上前车
                b.is_serving=-1
